...
export default function NewsletterSignup({
  ...
  description = "Join 1000+ students getting deadlines, scholarship news and visa tips every week. No spam, unsubscribe any time.",
  cta = "Get the free guide",
  highlight = "📚 Free PDF: Pakistan-to-Germany Application Checklist",
}) {
      ...
      <div className="rounded-xl border border-brand-200 bg-brand-50 p-5">
        ...
        <form onSubmit={handleSubmit} className="mt-3 flex flex-col gap-2 sm:flex-row">
          ...
          <button
            ...
            className="rounded-lg bg-brand-500 px-4 py-2 text-sm font-bold text-ink-900 transition hover:bg-brand-400 disabled:opacity-60"
          >
            {status === "submitting" ? "..." : cta}
          </button>
        </form>
        {status === "error" && (
          <p className="mt-2 text-xs text-rose-600">Couldn't subscribe — please try again.</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-gradient-to-br from-brand-50 via-amber-50 to-white p-8 shadow-md sm:p-10">
      {highlight && (
      ...
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-3 sm:flex-row">
        <input
          ...
          className="flex-1 rounded-lg border border-slate-300 bg-white px-4 py-3 text-base outline-none focus:border-brand-400 focus:ring-2 focus:ring-brand-100"
        />
        <button
          type="submit"
          disabled={status === "submitting"}
          className="rounded-lg bg-brand-500 px-6 py-3 font-bold text-ink-900 shadow-md transition hover:bg-brand-400 disabled:opacity-60"
        >
      ...
      {status === "error" && (
        <p className="mt-3 text-sm text-rose-600">
          Couldn't subscribe — please try again or email us directly.
        </p>
      )}
